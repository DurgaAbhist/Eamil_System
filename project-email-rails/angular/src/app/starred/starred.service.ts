...
export class StarredService {
         ...
         starredChanged = new Subject<Inbox[]>();
         ...
         getMails() {
            return this.starredMails;
         }

         getMail(index: number) {
            return this.starredMails[index];
         }

         updateMails(newMail: Inbox) {
            this.starredMails.push(newMail);
            this.starredChanged.next(this.starredMails.slice());
         }
