...
export class ImportantService {
   ...
   importantChanged = new Subject<Inbox[]>();

   importantMails: Inbox[] = [
      new Inbox(
         'Richard',
   ...
   getMails() {
      return this.importantMails;
   }

   getMail(index: number) {
      return this.importantMails[index];
   }

   updateMails(newMail: Inbox) {
      this.importantMails.push(newMail);
      this.importantChanged.next(this.importantMails.slice());
   }
}
