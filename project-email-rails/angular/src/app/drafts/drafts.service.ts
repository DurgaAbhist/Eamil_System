...
export class DraftsService {
   ...
   draftsChanged = new Subject<Inbox[]>();
   ...
   getMails() {
      return this.draftsMails;
   }

   getMail(index: number) {
      return this.draftsMails[index];
   }

   updateMails(newMail: Inbox) {
      this.draftsMails.push(newMail);
      this.draftsChanged.next(this.draftsMails.slice());
   }
}
